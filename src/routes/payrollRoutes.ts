...
router.put('/:id/items', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (period) {
      if (periodStart) period.periodStart = periodStart;
      if (periodEnd) period.periodEnd = periodEnd;
      if (payDate) period.payDate = payDate;
      if (status) period.status = status;
      if (Array.isArray(items)) {
        period.items = items;
        period.totalHours = items.reduce((acc, item) => acc + (Number(item.totalHours) || 0), 0);
        period.totalGrossPayroll = items.reduce((acc, item) => acc + (Number(item.grossPay) || 0), 0);
        period.totalDeductions = items.reduce((acc, item) => acc + (Number(item.deductions) || 0), 0);
        period.totalNetPayroll = items.reduce((acc, item) => acc + (Number(item.netPay) || 0), 0);
      }
      await mySQLDb.savePayrollPeriod(period);
    }
    res.json({ success: true, period });
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  try {
    await mySQLDb.deletePayrollPeriod(id);
    db.deletePayrollPeriod(id);
    res.json({ success: true, message: 'Payroll period deleted successfully' });
  } catch (err: any) {
    db.deletePayrollPeriod(id);
    res.json({ success: true, message: 'Payroll period deleted from local storage' });
  }
});
